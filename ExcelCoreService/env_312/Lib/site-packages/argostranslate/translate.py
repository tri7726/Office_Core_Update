...
class PackageTranslation(ITranslation):
    ...
    def __init__(self, from_lang: Language, to_lang: Language, pkg: Package):
        ...
        if settings.chunk_type in [settings.ChunkType.ARGOSTRANSLATE, settings.ChunkType.DEFAULT]:
            if "stanza" in str(pkg.packaged_sbd_path) and sbd.stanza is not None:
                Sentencizer = StanzaSentencizer
            else:
                Sentencizer = MiniSBDSentencizer # Default to MiniSBD if stanza is missing or not applicable
