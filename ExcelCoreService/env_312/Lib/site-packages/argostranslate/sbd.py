...
# Mock minisbd to avoid onnxruntime dependency in PyInstaller
import re
class MockSBDetect:
...
SBDetect = MockSBDetect
minisbd_models = MockModels()
...
class StanzaSentencizer(ISentenceBoundaryDetectionModel):
    ...
    def lazy_pipeline(self):
        if self.stanza_pipeline is None:
            if stanza is None:
                raise RuntimeError("Stanza is not installed. Install stanza (and torch) or use MiniSBD/SpaCy.")
